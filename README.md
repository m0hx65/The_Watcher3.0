...
| `/probe <user>` | Test every profile source against one account and report which answer — the API, the ID route, the public page (with status and byte count), and the media downloader. The fast way to tell a blocked gate from a broken route |
