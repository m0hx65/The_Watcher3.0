...
| | |
|---|---|
| 🏠 **A fourth door: your own PC** | Instagram now hands the logged-out profile page — the one with the follower/following counts, bio and privacy flag — only to IPs it hasn't seen scraping. Render's and Cloudflare's are both bounced to the login page with a `429`; your home line or VPN gets it every time (measured 2026-09-05). `tools/home_fetcher` is a 150-line service you run on your PC, exposed for free with one `tailscale funnel` command; the bot asks it for that page only, after its own attempt was refused, and parses the same payload. PC off? The sweep stays id-only for that run and says so. |
...
| `HOME_FETCH_URL` | — | Public URL of [`tools/home_fetcher`](tools/home_fetcher/README.md) running on your PC (e.g. a Tailscale Funnel URL). Asked for the public profile page only, after this host's own request was refused. Empty = disabled |
| `HOME_FETCH_TOKEN` | — | The shared secret the home fetcher prints at startup; sent as `X-Watcher-Token` |
