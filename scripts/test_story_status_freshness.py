...
The status is now announced only when it CHANGES, and never on top of the media
that already announced it.
...
- an unchanged status is silent, and delivered media is the only message,
- a manual Recheck still always answers, and STORY_STATUS_HEARTBEAT restores
  the old every-sweep line,
...
async def test_blocked_check_never_reports_stored_status() -> None:
    ...
    # The sweep summary already names every account it couldn't check, so the
    # per-account notice is logged, not sent.
    expect("a blocked sweep says nothing per-account", texts == [], str(texts))
    async with get_session() as session:
...
async def test_transitions_alert_once() -> None:
    ...
    # Same story still up on the next sweep — nothing has changed, so nothing
    # is said. This is the message-per-sweep spam the owner was drowning in.
    texts = await _story_phase(service, account_id, "target", _reel(story=True))
    ...
    expect("a new story alerts once",
           texts == [t for t in texts if "just posted a story" in t] and len(texts) == 1,
           str(texts))
...
async def test_delivered_story_is_one_message() -> None:
    ...
    # The status baseline still moved, so the next sweep stays quiet too.
    texts = await _story_phase(service, account_id, "poster", _reel(story=True))
...
async def test_heartbeat_mode_restores_the_old_line() -> None:
    account_id = await _new_account("heartbeat")
    instagram = FakeInstagram()
    service = _make_service(instagram)

    await _story_phase(service, account_id, "heartbeat", _reel(story=True))
    settings.story_status_heartbeat = True
    try:
        texts = await _story_phase(
            service, account_id, "heartbeat", _reel(story=True)
        )
    finally:
        settings.story_status_heartbeat = False
    expect("heartbeat mode reports the status every sweep",
           any("HAS STORY" in t for t in texts), str(texts))
...
async def main() -> int:
    ...
    await test_heartbeat_mode_restores_the_old_line()
